...
//void malloc_check(void* m) {
//    if(!m){
//        logger(ERROR, "[ERROR] MALLOC_CHECK");
...
int64_t
bucket_chaining_join_with_sealing_buffer(const struct table_t *const R,
                                         const struct table_t *const S,
                                         struct table_t *const tmpR,
                                         output_list_t **output)
{
    ...
    tuple_t * output_buf = (tuple_t*) malloc(SEAL_BUF_RECORDS * sizeof(tuple_t));
    ...
    for(uint32_t i=0; i < numS; i++ ){
        ...
        for(int hit = bucket[idx]; hit > 0; hit = next[hit-1]){
            ...
            if(Stuples[i].key == Rtuples[hit-1].key){
                ...
                output_buf[in_buffer].key = Stuples[i].key;
                output_buf[in_buffer].payload = Stuples[i].payload;
                in_buffer++;
                if (in_buffer >= SEAL_BUF_RECORDS) {
                    uint32_t output_size = SEAL_BUF_RECORDS * sizeof(tuple_t);
                    uint32_t sealed_buffer_size =
                            sgx_calc_sealed_data_size((uint32_t)strlen(aad_mac_text), output_size);
                    sealed_buffer = (uint8_t*) malloc(sealed_buffer_size);
                    ...
                    if (err != SGX_SUCCESS)
                    {
                        logger(DBG, "Sealing error: %d", err);
                    }
                    in_buffer = 0;
                }
#endif
 ...
 * Histogram-based hash table build method together with relation re-ordering as
 * described by Kim et al. It joins partitions Ri, Si of relations R & S. 
 * This version includes SIMD and prefetching optimizations as described by Kim et al. 
 * The parallel radix join implementation using this function is PRHO. 
 * Note: works only for 32-bit keys.
 */
...
#warning SIMD comparison for 64-bit keys are not implemented!
    ...
    /*uint32_t intervall_bigger_than_two = 0;
    uint32_t intervall_equal_four = 0;
    uint32_t intervall_equal_three = 0;
    uint32_t intervall_equal_two = 0;
    //#intervall_bigger_than_two = 192, #intervall_equal_four =0, #intervall_equal_three = 192, #intervall_equal_two = 128 
    */
    for( uint32_t i = 0; i < numS/PROBE_BUFFER_SIZE; i ++ ) {
        ...
        for( int k = 0; k < PROBE_BUFFER_SIZE; k++ ) {
            ...
            /*if(( end-j) != 2){
                if(( end-j) == 4){
                    intervall_equal_four++;
                }
                if(( end-j) == 3){
                    intervall_equal_three++;
                }
                intervall_bigger_than_two ++;
                //logger(INFO, "j=%d, end=%d\n",j,end);
            }
            else{
                intervall_equal_two ++;
            }*/
    ...
    //This part should be scalar, not vectorized like in the first Balkesen code.
    for(uint32_t i = numS - (numS % PROBE_BUFFER_SIZE); i < numS; i++ ){
        const uint32_t idx  = HASH_BIT_MODULO(Stuples[i].key, MASK, NUM_RADIX_BITS);
    ...
    //logger(INFO, "#intervall_bigger_than_two = %d, intervall_equal_four =%d, intervall_equal_three =%d, intervall_equal_two = %d \n",intervall_bigger_than_two, intervall_equal_four, intervall_equal_three, intervall_equal_two );
    /* clean up */
    free(hist);
 ...
 * described by Kim et al. It joins partitions Ri, Si of relations R & S. 
 * This version includes SIMD and prefetching optimizations as described by Kim et al. 
 * The modified version uses now AVX 256 bits instead of SSE 128 bits originally. 
 * The parallel radix join implementation using this function is PRHO. 
 * Note: works only for 32-bit keys.
 * Warning: PRHO_avx works only correctly, if the inner R table is bigger than the outer S table.
 */
int64_t
histogram_optimized_join_avx(const relation_t * const R, 
                         const relation_t * const S,
                         relation_t * const tmpR,
                         output_list_t ** output)
{
...
#warning SIMD comparison for 64-bit keys are not implemented!
...
#endif
    uint32_t PROBE_BUFFER_SIZE_AVX = PROBE_BUFFER_SIZE * 2;
    intkey_t key_buffer[PROBE_BUFFER_SIZE_AVX];
    uint32_t hash_buffer[PROBE_BUFFER_SIZE_AVX];
    ...
    //uint32_t intervall_equal_four = 0;
    //uint32_t intervall_equal_two = 0;
    for( uint32_t i = 0; i < numS/PROBE_BUFFER_SIZE_AVX; i ++ ) {

        for( int k = 0; k < PROBE_BUFFER_SIZE_AVX; k++ ) {
            const intkey_t skey = Stuples[i * PROBE_BUFFER_SIZE_AVX + k].key;
            const uint32_t idx = HASH_BIT_MODULO(skey, MASK, NUM_RADIX_BITS);
            /* now we issue a prefetch for element at S[idx] */
            prefetch(tmpR->tuples + hist[idx]);
            key_buffer[k]  = skey;
        ...
        for( int k = 0; k < PROBE_BUFFER_SIZE_AVX; k++ ) {
            ...
            /* SIMD comparisons in groups of 4 (8B x 4 = 256 bits) */
            int     j          = hist[hash_buffer[k]];
            int     end        = hist[hash_buffer[k]+1]; //hash_buffer[k]+1
            __m256i search_key = _mm256_set1_epi32(key_buffer[k]);

            for( ; j < end; j += 4) {

                __m256i keyvals = _mm256_loadu_si256((__m256i const *)(tmpRtuples + j));
                /*if(( end-j) == 3){
                    keyvals =  _mm256_insert_epi32 (keyvals, -1, 6);
                }*/
                keyvals         = _mm256_cmpeq_epi32(keyvals, search_key);
                counter         = _mm256_add_epi32(keyvals, counter);
                /* TODO: we're just counting, not materializing results */
            }
        }
    }
    ...
    //This part should be scalar, not vectorized like in the first Balkesen code.
    for(uint32_t i = numS - (numS % PROBE_BUFFER_SIZE_AVX); i < numS; i++ ){
    ...
    match += -( _mm256_extract_epi32(counter, 0) + _mm256_extract_epi32(counter, 2)
    + _mm256_extract_epi32(counter, 4) + _mm256_extract_epi32(counter, 6) );
    //logger(INFO, "#intervall_bigger_than_two = %d, intervall_equal_four =%d, intervall_equal_two = %d \n",intervall_bigger_than_two, intervall_equal_four, intervall_equal_two );
...
//    cpu_set_t set;
    ...
    for(i = 0; i < nthreads; i++){
        ...
        rv = pthread_create_cpuidx(&tid[i], NULL, cpu_idx, prj_thread, (void*)&args[i]);
#else
...
//#endif
    ...
    for( i=0; i < (1<<NUM_RADIX_BITS); i++ ){
        ...
        if(R_count_per_cluster[i] > 0 && S_count_per_cluster[i] > 0){
            ...
            result += bucket_chaining_join(&tmpR, &tmpS, NULL, nullptr);
        }
        else {
            r += R_count_per_cluster[i];
            s += S_count_per_cluster[i];
    ...
    ocall_stopTimer(&timer2);/* build finished */
    ocall_stopTimer(&timer1);/* probe finished */
    ocall_get_system_micros(&end);
    /* now print the timing results: */
    print_timing(timer1, timer2, timer3, relR->num_tuples + relS->num_tuples, result, start, end, 0 ,0);
#endif
...
{
    logger(DBG, "Sealing buffer number of records = %d", SEAL_BUF_RECORDS);
    return join_init_run(relR, relS, bucket_chaining_join_with_sealing_buffer, nthreads);
}
...
result_t *
PRHO_avx(relation_t * relR, relation_t * relS, int nthreads)
{
    return join_init_run(relR, relS, histogram_optimized_join_avx, nthreads);
}