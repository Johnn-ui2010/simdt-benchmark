...
//SIMD version:
struct thr_arg_t_simd
{
    int tid;
    CHTJoin_simd *chtJoin;
};

static void *run_simd(void *args)
{
    thr_arg_t_simd *arg = reinterpret_cast<thr_arg_t_simd *>(args);
    arg->chtJoin->join(arg->tid);
    return NULL;
}

template <int numbits>
join_result_t CHTJ_simd(relation_t *relR, relation_t *relS, int nthreads, int impl_type)
{
    tuple_t *output = NULL;
    //	posix_memalign((void**)(&output), 64, relR->size * sizeof(tuple_t));
    output = (tuple_t *)memalign(64, relR->num_tuples * sizeof(tuple_t));
    ...
    //	numa_localize(output, relR->size, nthreads);
    logger(INFO,"in CHTJoinWrapper: impl_type=%d.\n",impl_type);
    CHTJoin_simd *chtJoin = new CHTJoin_simd(nthreads, 1 << numbits, relR, relS, output, impl_type);
    pthread_t *threads = new pthread_t[nthreads];
    //    pthread_attr_t attr;
    //    cpu_set_t set;

    thr_arg_t_simd *args = new thr_arg_t_simd[nthreads];
    //    pthread_attr_init(&attr);
...
#else
        (void)(cpu_idx);
        int rv = pthread_create(&threads[i], NULL, run_simd, (void *)&args[i]);
#endif
        if (rv)
        {
            //            printf("[ERROR] return code from pthread_create() is %d\n", rv);
...
template join_result_t CHTJ_simd<7>(relation_t *relR, relation_t *relS, int nthreads, int impl_type);
